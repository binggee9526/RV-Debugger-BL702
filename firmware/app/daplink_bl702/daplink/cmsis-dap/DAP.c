...
uint32_t DAP_ExecuteCommand(const uint8_t *request, uint8_t *response) {
  ...
  /* ID_DAP_ExecuteCommands (0x7F) and ID_DAP_QueueCommands (0x7E) both
   * pack multiple DAP commands into one USB packet.  Keil MDK uses
   * ID_DAP_QueueCommands for flash programming; OpenOCD uses
   * ID_DAP_ExecuteCommands.  In a synchronous (bare-metal) firmware both
   * are handled identically — execute immediately and return the combined
   * response. */
  if ((*request == ID_DAP_ExecuteCommands) || (*request == ID_DAP_QueueCommands)) {
    *response++ = *request++;
    cnt = *request++;
